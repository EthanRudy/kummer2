...
//  January 16, 2000
//
//  Implements the graphical display for the Fish Simulation
...
{
  apvector<color> watercolor(5);
  watercolor[0] = LIGHTBLUE;
  watercolor[2] = DARKSEAGREEN;
  watercolor[4] = SEAGREEN;
  watercolor[3] = MEDIUMSEAGREEN;
  watercolor[1] = LIGHTSEAGREEN;
  ...
  for (r = 0; r < rows; r++)
  {
    for (c = 0; c < cols; c++)
    {
      if(env.NumAlgaeAt(Position(r,c)) <= 4)
      {
        myWin.SetPen(watercolor[env.NumAlgaeAt(Position(r,c))], 2);
        myWin.SetBrush(watercolor[env.NumAlgaeAt(Position(r,c))]);
      }
      else
      {
        myWin.SetPen(watercolor[4], 2);
        myWin.SetBrush(watercolor[4]);
      }
