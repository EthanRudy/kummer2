...
int main()
{
 int iX, iY;
    ...
    for(dScale = 1.0; dScale < 2.0; dScale += 0.01)
    {
        ...
        testWindow.DrawImage(testImage, 10, 10, testImage.GetWidth() * dScale, testImage.GetHeight() * dScale);
    ...
    testWindow.StoreImage(testImage, 0, 0, testWindow.GetWidth() - 1, testWindow.GetHeight() - 1);
    ...
    for(dScale = 1.0; dScale > 0.0; dScale -= 0.01)
    {
        ...
        // Scale and draw the image
        testWindow.DrawImage(testImage, 0, 0, testImage.GetWidth() * dScale, testImage.GetHeight() * dScale);
    ...
    while(testWindow.GetKeyPress(cKeyData) != ESCAPE)
    {
       ...
       // Dragging voodoo
        if(bDragging == false) {
            if(testWindow.GetButtonState(LEFT, iX, iY) == BUTTON_DOWN) {
            ...
            if(testWindow.GetButtonState(LEFT, iX, iY) == BUTTON_UP) {
                bDragging = false; 
            } else {
                if(iX != iXOld) {
	...
	testWindow.SetBuffering(false);
 	...
 	testWindow.SetPen(BLACK);
	testWindow.SetFont(16, BOLD, SWISS);
	testWindow.DrawString(5, testWindow.GetHeight() - 25, "Show's over!  Activate Console window and hit <Enter> to quit.");
	testWindow.UpdateBuffer();
